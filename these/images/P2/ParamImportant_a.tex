...
\draw (\Zmiroir+\epmiroir/2,\vmax*4/5)--(\Zmiroir+3*\epmiroir/2,\vmax)
 node[right]{\mimo};
\draw[red,-latex',line width=1pt](\Zmiroir+\epmiroir+\eps, \vmiroir)--+(\vmiroir/5,0);
%  node[above right]{\footnotesize $\Vmir$};
...
\draw[-latex'] (0,\vzero)++(\deltaz,-0)++(\eps,0)--++({(\vzero-0)/5},0) node[below]{$\vinj$};
%\draw[-latex'] (0,\vzero)++(\deltaz,+\deltav)++(\eps,0)--++({(\vzero+\deltav)/10},0);
