...
\draw[blue, opacity=0.3, dashed](\Zmiroir+\tdeuxfig*\vmiroir,-10)--+(0,20);
\draw[latex'-latex'](\Zmiroir+\tunfig*\vmiroir, 4/5*\vmax)--(\Zmiroir+\tdeuxfig*\vmiroir, 4/5*\vmax)
node[above=-2pt,pos=0.5]{$\Lcol$};

\foreach \tt/\ttext in {\tdeuxfig/,\trelance/\p $n+1$}
{
\def\Tun{\tt}
\draw[fill=blue!60] 
...
--++({\Tdeux*2*\deltav},2*\deltav) node[above left=-2pt]{\p $n$ ralenti}
--++(2*\deltaz,0)
